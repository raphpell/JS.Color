Color =(function(){
	...
	, inRange= (n,s)=> (isNaN(n)||!MAX[s])?null:(n>MAX[s]?MAX[s]:(n<0?0:n))
	, isHEX= s =>{
		if(s[0]=='#')s=s.slice(1)
	...
	, is= (sMode,X,Y,Z,A)=>{
		let o =isNaN(Y)?parseString(X,sMode):getObject(sMode,[0+X,0+Y,0+Z,A])
		if( o ){
	...
	, f2=HEXtoDEC= s =>{
		let n=0
		,f= n => '0123456789ABCDEF'.indexOf(s.charAt(n))
	...
	, HSVtoHSL= o =>{
		let L=(2-o.s/100)*o.v/2, S=o.s*o.v/(L<50?L*2:200-L*2)
		if(isNaN(S)) S=0
